...
void merge_add(const vif::vec<1,TX1>& x1, const vif::vec<1,TX2>& x2,
    const vif::vec<1,TY1>& y1, const vif::vec<1,TY2>& y2,
    vif::vec<1,TX>& x, vif::vec<1,TY>& y) {

    vif_check(x1.dims == y1.dims, "incompatible dimensions between X1 and Y1 (",
        x1.dims, " vs. ", y1.dims, ")");
    vif_check(x2.dims == y2.dims, "incompatible dimensions between X2 and Y2 (",
        x2.dims, " vs. ", y2.dims, ")");

    vif::uint_t n1 = x1.size(), n2 = x2.size();
    x.clear(); x.reserve(n1+n2);
    y.clear(); y.reserve(n1+n2);

    vif::uint_t i1 = 0, i2 = 0;
    while (i1 < n1 || i2 < n2) {
        if (i1 == n1) {
            ...
            if (x1.safe[i1] < x2.safe[i2]) {
                ...
                if (i2 == 0) {
                    ...
                    y.push_back(y1.safe[i1] + vif::interpolate(
                ...
                if (i1 == 0) {
                    ...
                    y.push_back(y2.safe[i2] + vif::interpolate(
