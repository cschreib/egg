...
\item \bashinline{GAIN}: sets the conversion factor from electrons/seconds to map units. If you cannot figure out this value, set it to one.
\item \bashinline{BACK_MAG}: this is the background surface brightness, which must be given in ``magnitudes per arcsec$^2$''. Together with the exposure time, this value defines the amplitude of the background Poissonian noise.
