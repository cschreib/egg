...
\item Open a terminal and navigate to the \cppinline{phypp-master} directory. Then, if you are using cfitsio and WCSlib from your package manager, run the following commands:
\begin{minted}{bash}
...
The ``...'' have to be replaced by the actual directory in which each library was installed. For example, if you have installed cfitsio in the \cppinline{/opt/local/share/cfitsio} directory, then the ``..." after \cppinline{-DCFITSIO_ROOT_DIR} in the above command has to be replaced by \cppinline{"/opt/local/share/cfitsio"}.

Another thing to consider is that, by default, CMake will try to install the library inside the system default directories. If you have root access to your computer, this is the recommended way to go as it will make things simpler. If you do not have root access, or if for some reason you prefer to install the library somewhere else than the default location, you can specify manually the install directory using \cppinline{-DCMAKE_INSTALL_PREFIX}:
\begin{minted}{bash}
mkdir build && cd build
cmake ../ -DCMAKE_INSTALL_PREFIX=...
\end{minted}

Here, the ``...'' have to be replaced by the directory in which you want to install the library. For example, if you want to install it in \cppinline{/opt/local}, just replace ``...'' by \cppinline{"/opt/local"}. This will install the library headers in \cppinline{/opt/local/include}. This can of course be combined with the manual installation directories for WCSlib and cfitsio.
...
\item Using the terminal, navigate now inside the \cppinline{ifni-master} directory. Similarly, run the following commands:
\begin{minted}{bash}
mkdir build && cd build
cmake ../
\end{minted}
...
The \cppinline{ifni} binary will be created in \cppinline{../bin}. See, not that hard!

\subsection{Making sure everything works}

Navigate into the \cppinline{ifni-master/bin} directory and call:
\begin{minted}{bash}
...
Also, a file called \bashinline{ifni-2015xxxx.fits} (e.g., for me it was \bashinline{ifni-20150323.fits}) weighting about 20MB will be created in the same directory. This is the output catalog, in FITS format. You can open it in IDL to check its content with the following IDL command: \\[0.5cm]
...
This should produce no output in the terminal, but create two files in the same directory, \cppinline{sky-f160w.cat}, which is the Skymaker catalog, and \cppinline{sky-f160w-hdr.txt}, which is the WCS header to feed to Skymaker.
